...
class OrdersController extends BaseController {
  ...
  async new ({ request, auth, response }) {
    ...
    await postFiles.moveAll(use('Helpers').publicPath(filePath), (post) => {
      const fileName = `${use('uuid').v1().replace(/-/g, '')}_${post.clientName}`
    ...
    try {
      order.calcTotalPrice(config.charge.commission)
  ...
  async index ({ request, response, auth, decodeQuery }) {
    ...
    const countQueryBuilder = Order.query(decodeQuery().countQuery)
    let whereQuery = {}
    if (user.role === 'admin') {
      switch (orderType) {
        case 'purchase':
  ...
  async pay ({ request, response, auth, instance }) {
    ...
    if (perm.isAllowed) {
      ...
      try {
        const seller = await order.seller().fetch()
        const chargeRate = $n(config['charge']['commission'])
        const payoutAmount = ($n(order.subtotal) * $n(1 - chargeRate / 100)).toFixed(2)
  ...
  async rate ({ request, response, auth, instance }) {
    ...
    if (perm.isAllowed) {
      const ratingData = request.only(['communication', 'professionalism', 'recommendation'])
  ...
  async checkPermission (order, user, action) {
    ...
    const ownership = order.relationship(user)
