...
class InstaaccountsController extends BaseController {
  ...
  async create({ request, auth, response }) {
    ...
    const verfiedAccountExists = await Instaaccount.where({ verified: true })
      .where({ username })
      .count();
    if (verfiedAccountExists) {
      return response.validateFailed("instagram_already_exists");
    }
    const appliedAccountExists = await Instaaccount.where({
      username: username,
    })
  ...
  async validateInstagram({ request, auth, response, instance }) {
    ...
    if (
      user.role != "admin" &&
      user._id.toString() != instance.user_id.toString()
    ) {
      throw UnAuthorizeException.invoke();
    }
    const isValid = await this.validateInsta(
      instaaccount.username,
      instaaccount.verification_code
    );
    if (isValid) {
  ...
  async uploadInsights({ request, auth, instance, response }) {
    ...
    const fileName = `${use("uuid").v1().replace(/-/g, "")}_${
      image.clientName
    }`;
    ...
    const s3Url = await Drive.disk("s3").put(
      `${filePath}/${fileName}`,
      Drive.disk("local").getStream(`${filePath}/${fileName}`)
    );
  ...
  async adminList({ request, auth, instance, response }) {
    ...
    const instaaccounts = await Instaaccount.query()
      ...
      .limit(parsedQuery.limit)
      .fetch();
    const total = await Instaaccount.query().where(parsedQuery.where).count();
    return response.apiCollection(instaaccounts, { total });
  }
  ...
  async getInstaInfoRaw({ request }) {
    const username = request.input("username");
    try {
      return await CrawlerManager.getInstaInfo(username);
    } catch (e) {
      console.error(e);
      return null;
    }
  ...
  async getInstaInfo(username) {
    try {
      const graphql = await CrawlerManager.getInstaInfo(username);
    ...
    if (minPrice || maxPrice) {
      if (minPrice) {
        where["product.categories"] = {
          $elemMatch: { "pricing.price": { $gt: minPrice } },
        };
      }
      if (maxPrice) {
        where["product.categories"] = {
          $elemMatch: { "pricing.price": { $lt: maxPrice } },
        };
      }
    }
    if (gender) {
      if (gender === "female") {
        where["demographics.gender"] = {
          $elemMatch: { name: "Women", percent: { $gt: 50 } },
        };
      } else {
        where["demographics.gender"] = {
          $elemMatch: { name: "Men", percent: { $gt: 50 } },
        };
      }
    }
    if (countries) {
      where["demographics.country"] = {
        $elemMatch: { name: { $in: countries } },
      };
    }
    if (niches) {
