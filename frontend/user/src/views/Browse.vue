...
<script>
   ...
   const defaultState = {
      UsdCircle,
      niches: niches.map((niche) => {  return { ...niche, checked: false } }),
      categories: categories.map((c) => { return { ...c, checked: false } }),
      genders: genders.map((g) => { return { ...g, selected: false } }),
      countries: countries,
      filter: {
         price: {
            min: null,
   ...
   export default {
      ...
      data() {
         return {
            ...
            UsdCircle,
            niches: defaultState.niches,
            categories: defaultState.categories,
            genders: defaultState.genders,
